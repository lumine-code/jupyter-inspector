...
function consumeJupyterKernel(jupyterProvider) {
  ...
  let disposed = false;
  const connect = () => {
    if (!disposed) session.setProvider(jupyterProvider);
  };
...
module.exports = {
  provideBackgroundTips() {
    return {
      packageName: "jupyter-inspector",
      tips: [
        "You can look up the documentation for whatever is under the cursor with {{ 'jupyter-inspector:inspect' | keystroke }}",
      ],
    };
  },
