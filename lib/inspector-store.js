...
// Replace the temporary's name in every textual mime representation. The name
// is a single identifier token, so it survives IPython's colouring in one
// piece and a plain string replacement is safe.
function scrubBundle(bundle, { from, to }) {
  const out = {};
  for (const [mime, value] of Object.entries(bundle || {})) {
    out[mime] = typeof value === "string" ? value.split(from).join(to) : value;
  }
  return out;
}
...
class InspectorStore {
  ...
  // back to the expression it stands for, in every textual representation.
  _inspectExpression(requestId, expression, cursorPos, onDone = null, scrub = null) {
    this.kernel.inspect(expression, cursorPos).then((result) => {
      ...
      if (!result.found) {
        ...
        this.setBundle(scrub ? scrubBundle(result.data, scrub) : result.data);
      }
    });
  }
