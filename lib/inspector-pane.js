...
class InspectorPane {
  constructor(session, watchEditor) {
    ...
    this.disposer = new CompositeDisposable(
      ...
      lumine.commands.add(this.element, {
        "jupyter-inspector:scroll-up": {
          description: "Scroll the inspection up.",
          didDispatch: (event) => this.scroll(event, -1),
        },
        "jupyter-inspector:scroll-down": {
          description: "Scroll the inspection down.",
          didDispatch: (event) => this.scroll(event, 1),
        },
      }),
    );
  ...
  // Inspection results and their kernels are live session objects. Persist
  // only the pane's presence and let the restored session start empty.
  serialize() {
    return { deserializer: "jupyter-inspector/InspectorPane" };
  }
